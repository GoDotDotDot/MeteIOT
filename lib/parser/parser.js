...
class MeteParser extends Emitter {
  constructor (opts) {
    ...
    if (typeof opts.stationId !== 'string') {
      throw Error(`staionId is required and the type must be string,but get ${typeof opts.stationId}`)
    }
  ...
  sendCommand (id, command) {
    if (!this.devices[id]) {
      throw Error(`the device(id:${id}) is not be registed!`)
    }
