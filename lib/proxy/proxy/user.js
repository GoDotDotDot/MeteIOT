...
exports.getStationUserByAccount = async function (stationName, account) {
  const doc = await StationUser.findOne({stationName, account})
  return doc
}

exports.stationUserLogin = async function (stationName, account, password) {
  const user = await this.getStationUserByAccount(stationName, account)
  if (!user) return {isMatch: false, message: '没有找到该用户'}
  const rst = await this.authPassword(user, password)
  return rst
}

exports.stationUserRegister = async function (stationName,account, password) {
  const doc = new StationUser({stationName,account, password})
